...
for threshhold=0.1:0.02:0.25% scan various of thresholds for edge detection in the segmentation part of the recogntion
      ...
      if ~(inpc==1)
       ...
       %............................................
           [  vessel_cont, symmetry_score, borderxy, imborder, YaxisSize]=Find_Vessel_Contour(filename,Symmetry_Mode, (ysize^2)/2,threshhold,Segmentation_Mode); % extract object from image after resizing it to y size and using canny threshold  of threshhold use 'BORDER_CANNY' segmentation mode for image (mode base on finding outer border of objects) return border of object found in image and its symmetry level of found object wich is the use to estimate accuracy
             ...
             %----------------------------------------get user input regarding result---------------------------------------------------------
                          imshow(imborder);
              
              disp('Enter 1 to keep this result and finish scan');  
              disp('Enter 2 to keep the result but continue scan'); 
              disp('Enter 3 to reject this result and continue scan');
              inpc=input('Press key and then Enter:');
              if isempty(inpc) inpc=3; end;
            %  k=0;
             %  while ~k 
              %    k=waitforbuttonpress;
               %   inpc=get(gcf,'currentcharacter')
               % end
             %----------------------------------------check  the quality of the result and decide wether to keep or descard based pm its symmetry_score----------------------------------------------------------------------------
             if inpc==2 || inpc==1 ||   best_symmetry_score<0  % the symmetry of the result is used to evaluate its quality result will be saved only if its score is higher then previous best results
...
Algortihm:
Scan various of threshold and sizes of for segmenting image in filename. The recognition itself done by Find_Vessel_Contour. This function scan Find_Vessel_Contour with various of different image
size for the image in filename and with various of threshold for the edges in the segmentation step. It pick the best result according to user choice (user input).
%}
